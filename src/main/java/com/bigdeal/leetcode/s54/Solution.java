    ...
    private void helper(int[][] matrix, int topRow, int bottomRow, int leftCol, int rightCol, Direction direction, List<Integer> result) {
        ...
        switch (direction) {
            ...
            case DOWN -> {
                ...
                helper(matrix, topRow, bottomRow, leftCol, rightCol - 1, Direction.LEFT, result);
            }
            case LEFT -> {
                int j = rightCol;
                while (j >= leftCol) {
                ...
                helper(matrix, topRow, bottomRow - 1, leftCol, rightCol, Direction.UP, result);
            }
            case UP -> {
                int i = bottomRow;
                while (i >= topRow) {
                ...
                helper(matrix, topRow, bottomRow, leftCol + 1, rightCol, Direction.RIGHT, result);
            }
        }
